...
void CLK_Config()
{
	RCC->CR |= 0x00030000;  //HSEON ENABLE
...
void EXTI1_IRQHandler()
{
	if(EXTI->PR & (1 << 1))
	{
		...
		do
		{
			GPIOD->ODR = 0x00002000;  // sadece 13. led yanacak
			delay(1680000);
			GPIOD->ODR = 0x00000000;  // sadece 13. led yanacak
			delay(1680000);
			i++;
		}
		while(i<5);
...
void EXTI2_IRQHandler()
{
	if(EXTI->PR & (1 << 2))
	{
		...
		do
		{
			GPIOD->ODR = 0x00004000;  // sadece 14. led yanacak
			delay(1680000);
			GPIOD->ODR = 0x00000000;  // sadece 14. led yanacak
			delay(1680000);
			i++;
		}
		while(i<5);
